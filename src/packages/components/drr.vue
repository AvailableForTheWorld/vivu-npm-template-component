...
const stickMove = (e: MouseEvent) => {
  ...
  if (currentStick.value.includes('ro')) {
    ...
    if (onResize && onResize.value && onResize.value)
      setRect(onResize.value(getRect()))
...
const bodyMove = (ev: MouseEvent) => {
  ...
  if (onDrag && onDrag.value && onDrag.value)
    setRect(onDrag.value(getRect()))
...
const stickDown = (stick: any, ev: MouseEvent) => {
  if (!resizable.value || !active.value)
    return
